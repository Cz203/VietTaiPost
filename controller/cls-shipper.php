    ...
    public function toggleTrangThaiHoatDong($id, $trang_thai)
    {
        try {
            ...
            if ($result['count'] > 0) {
                return [
                    'success' => false,
                    'message' => 'Không thể thay đổi trạng thái vì shipper đang trong quá trình giao hàng'
                ];
            }

            // Kiểm tra trạng thái hiện tại
            $sql_check_status = "SELECT trang_thai FROM shipper WHERE id = ?";
            $stmt_check_status = $conn->prepare($sql_check_status);
            $stmt_check_status->execute([$id]);
            $current_status = $stmt_check_status->fetch(PDO::FETCH_ASSOC);

            // Xác định trạng thái mới
            $new_status = ($current_status['trang_thai'] == 'Đang hoạt động') ? 'Không hoạt động' : 'Đang hoạt động';
            ...
            if ($stmt->rowCount() > 0) {
                $message = $new_status == 'Đang hoạt động' ? 'Đã kích hoạt shipper' : 'Đã vô hiệu hóa shipper';
                return [
                    'success' => true,
                    'message' => $message
                ];
                ...
                return [
                    'success' => false,
                    'message' => 'Không thể cập nhật trạng thái shipper'
                ];
            }
        } catch (Exception $e) {
            return [
