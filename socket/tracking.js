...
function initializeTracking() {
  // Create WebSocket server for tracking
  const wss = new WebSocket.Server({ port: 8080 });
  console.log("WebSocket tracking server đang chạy tại ws://localhost:8080");
        ...
        db.query(
          ...
          (err, results) => {
            ...
            if (trang_thai === "Đang hoạt động") {
              ...
              console.log(
                `Shipper ${id} không hoạt động, không cập nhật vị trí`
              );
            }
          }
        );
